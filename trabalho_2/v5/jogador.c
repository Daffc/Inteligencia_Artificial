...
int criaChutes(PilhaFil *pilha, char *campo, int *saltos_pos, int pos_bola, char direcao){
  int saltos;
  int pos_atual;
  ...
  while(campo[pos_atual] == 'f'){
    ...
    while(campo[pos_atual] == 'f'){
      campo[pos_atual] = '.';
      empilhaFil(pilha, saltos + 1, pos_atual);
      pos_atual += direcao;
    }
  ...
  // printf("FIM CHUTES:\n");
  // printf("\tCAMPO FINAL: %s SALTOS: %d", campo, saltos);
  // imprimePilhaFil(pilha);
...
int minimax(int profundidade, int nivel, char jogadorMax, char *campo, Jogada *jogada, char *respostaFinal, int alpha, int beta){
  ...
  if(jogadorMax){
    ...
    do{
      ...
      inicializaPilha(&pilhaFil);
      saltos = criaChutes(&pilhaFil, campo_aux, saltos_pos, pos_bola, direcao);

      aux_pos_bola = pilhaFil.pilha[pilhaFil.topo].posicao + 1;
      ...
      while(saltos > 0 && !podaAlpha){
        ...
        if(!nivel){
          printf("+ MAX + SAL CAMPO:%s, VALOR: %d\n", campo_aux, result);
        }
        ...
        while(pilhaFil.pilha[pilhaFil.topo].salto == saltos && pilhaFil.topo != -1){
          entrada = desempilhaFil(&pilhaFil);
          campo_aux[entrada->posicao] = 'f';
        }

        aux_pos_bola = entrada->posicao -1;
